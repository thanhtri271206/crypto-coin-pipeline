...
with tab24h:
    ...
    with col_table:
        df_24h = df_no_stable[["symbol", "current_price", "price_change_percentage_24h",
                                "total_volume", "market_cap_rank", "rank_change"]].copy()
        ...
        styled = df_24h.style \
            .map(style_change, subset=["Change 24h %"]) \
...
with tab7d:
    ...
    with col_table7:
        df_t7 = df_no_stable[["symbol", "current_price", "price_change_percentage_7d_in_currency",
                               "market_cap_rank"]].copy()
        ...
        styled7 = df_t7.style \
            .map(style_change, subset=["Change 7d %"]) \
            .format({
                "Price (USD)": lambda x: f"${x:,.4f}" if x < 1 else f"${x:,.2f}",
...
with col_vol1:
    df_vol = df_raw[["symbol", "display_name", "total_volume", "avg_volume_7d",
                      "volume_spike_ratio", "price_change_percentage_24h"]].copy()
    ...
    def style_spike(val):
        if pd.isna(val): return "color: #6E7681"
        if val >= 3.0: return "color: #FFA726; font-weight: 700"
        if val >= 2.0: return "color: #FFA726"
        return "color: #8B949E"

    styled_vol = df_vol.style \
        .map(style_spike, subset=["Spike Ratio"]) \
        .map(style_change, subset=["Price Δ 24h %"]) \
        .format({
            "Volume 24h":  lambda x: f"${x/1e9:.2f}B" if x >= 1e9 else f"${x/1e6:.1f}M",
            "Avg Vol 7d":  lambda x: f"${x/1e9:.2f}B" if pd.notna(x) and x >= 1e9
...
df_rank = df_raw[["market_cap_rank", "symbol", "display_name", "current_price",
                   "price_change_percentage_24h", "total_volume", "rank_change", "fetched_at"]].copy()
df_rank = df_rank.sort_values("market_cap_rank")
df_rank["fetched_at"] = df_rank["fetched_at"].dt.strftime("%H:%M")
df_rank.columns = ["Rank", "Symbol", "Coin", "Price (USD)", "Change 24h %", "Volume 24h", "Rank Δ", "Time"]

styled_rank = df_rank.style \
    .map(style_change, subset=["Change 24h %"]) \
    .map(style_rank,   subset=["Rank Δ"]) \
    .format({
        "Price (USD)": lambda x: f"${x:,.6f}" if x < 0.001 else (f"${x:,.4f}" if x < 1 else f"${x:,.2f}"),
        "Change 24h %": lambda x: f"{x:+.2f}%" if pd.notna(x) else "N/A",
        "Volume 24h":   lambda x: f"${x/1e9:.2f}B" if x >= 1e9 else f"${x/1e6:.1f}M",
        "Rank Δ":       lambda x: f"{int(x):+d}" if pd.notna(x) and x != 0 else ("—" if x == 0 else "N/A"),
    })
